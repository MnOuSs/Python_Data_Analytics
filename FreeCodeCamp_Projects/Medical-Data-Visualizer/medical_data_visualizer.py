...
values_dict = {1:0, 2:1, 3:1}
df['cholesterol'] = df['cholesterol'].map(values_dict)
df['gluc'] = df['gluc'].map(values_dict)
...
def draw_heat_map():
  ...
  sns.heatmap(corr, mask=mask, annot=True, fmt='.1f', linewidths=1, cbar_kws={'shrink':.50, 'format': '%.2f'})
  # Do not modify the next two lines
